...
def _load_full_shadow_evidence(api, folder_id: str) -> dict:
    """Load only the exact historical evidence selected by the final PASS report.

    Historical full-shadow retries can leave orphan/HOLD SRC_* folders beside the
    final PASS evidence. Those folders remain immutable audit evidence, but they
    are not recovery membership authority. Membership comes exclusively from
    FULL_SHADOW_PARITY_FINAL.json -> source_summaries[] exact Drive pointers.
    """
    root = _list_children(api, folder_id)
    final_item = _unique(root, "FULL_SHADOW_PARITY_FINAL.json")
    ...
    if str(final.get("run_folder_id") or "") != folder_id:
        raise RuntimeError("RECOVERY_FULL_SHADOW_RUN_FOLDER_ID_MISMATCH")
    if final.get("generation_id") != FROZEN_GENERATION_ID:
        raise RuntimeError("RECOVERY_FULL_SHADOW_GENERATION_MISMATCH")
    if final.get("data_plane_impl_version") != FROZEN_IMPL_VERSION:
        raise RuntimeError("RECOVERY_FULL_SHADOW_IMPL_VERSION_MISMATCH")
    ...
    for summary in summaries:
        ...
        if (
            not expected_drive_id
            or not expected_name
            or not evidence_folder_id
            or not source_report_file_id
            or not isinstance(summary_stable, dict)
        ):
        ...
        if source_index < 1 or source_index in seen_source_indexes:
            raise RuntimeError(f"RECOVERY_FINAL_SOURCE_INDEX_INVALID_OR_DUPLICATE:{expected_name}:{source_index}")
        ...
        if stable != summary_stable:
            raise RuntimeError(f"RECOVERY_FINAL_STABLE_SOURCE_MISMATCH:{name}")
        if stable.get("generation_id") != FROZEN_GENERATION_ID or engine.get("generation_id") != FROZEN_GENERATION_ID:
            raise RuntimeError(f"RECOVERY_FINAL_SOURCE_GENERATION_MISMATCH:{name}")
        if stable.get("data_plane_impl_version") != FROZEN_IMPL_VERSION or engine.get("data_plane_impl_version") != FROZEN_IMPL_VERSION:
            raise RuntimeError(f"RECOVERY_FINAL_SOURCE_IMPL_VERSION_MISMATCH:{name}")
        stable_identity_ok = (
            str(stable.get("source_name") or "") == name
            and str(stable.get("source_drive_id") or "") == drive_id
            and int(stable.get("source_size_bytes") or -1) == int(source.get("size") or -2)
            and bool(str(stable.get("source_sha256") or ""))
            and bool(str(source.get("drive_md5") or ""))
        )
        if not stable_identity_ok:
            raise RuntimeError(f"RECOVERY_FINAL_STABLE_SOURCE_IDENTITY_MISMATCH:{name}")
    ...
    final_ids = {str(r.get("drive_id") or "") for r in summaries}
    ...
    compact = [
        {
            ...
            "physical": r["physical"],
            "semantic": r["semantic"],
            "historical_evidence_folder_id": r["historical_evidence_folder_id"],
            "historical_source_report_id": r["historical_source_report_id"],
        }
        for r in sources
    ]
    return {
        ...
        "evidence_fingerprint": _json_fingerprint(compact),
    }
