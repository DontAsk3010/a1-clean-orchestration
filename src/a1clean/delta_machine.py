...
from .semantic_work import discover_semantic_runtime, semantic_runtime_fingerprint
...
MACHINE_SCHEMA = "A1_GOVERNED_DELTA_MACHINE_V2_DUAL_STATE"
CHECKPOINT_SCHEMA = "A1_GOVERNED_DELTA_MACHINE_CHECKPOINT_V2_DUAL_STATE"
...
def _run_key(preflight: dict, persistent: dict, semantic_runtime: dict) -> str:
    return (
        f"{_github_sha()[:12]}_"
        f"{source_universe_fingerprint(preflight)[:12]}_"
        f"{persistent_state_fingerprint(persistent)[:12]}_"
        f"{semantic_runtime_fingerprint(semantic_runtime)[:12]}"
    )
...
def _persist_checkpoint(writer_api, checkpoint: dict) -> None:
    result = upload_json_payload(
        ...
        name=_checkpoint_name(
            int(checkpoint.get("checkpoint_sequence", 0)),
            str(checkpoint.get("status", "UNKNOWN")),
        ),
        payload=checkpoint,
    )
...
def _validate_checkpoint(
    ...
    *,
    ...
    expected = {
        ...
        "semantic_input_fingerprint": semantic_runtime_fingerprint(semantic_runtime),
...
def _find_or_create_run(
    ...
    *,
    ...
    run_key = _run_key(preflight, persistent, semantic_runtime)
    suffix = "_" + run_key
    folders = sorted(
        [
            item
            for item in _list_children(writer_api, FROZEN_PARITY_STAGING_FOLDER_DRIVE_ID)
            if item.get("mimeType") == FOLDER_MIME
    ...
    checkpoint = {
        ...
        "baseline_state_fingerprint": persistent_state_fingerprint(persistent),
        "semantic_input_fingerprint": semantic_runtime_fingerprint(semantic_runtime),
        "semantic_runtime_status": semantic_runtime.get("status"),
...
def _commit_operations(classification: dict, baseline_global: dict) -> dict:
    prior = {
        str(row.get("source_drive_id")): row
    ...
    for key_name, reason in (
        ...
        for row in classification.get(key_name, []):
            ...
            if sid and key not in seen:
                seen.add(key)
                purge.append(
                    {"source_drive_id": sid, "source_name": name, "reason": reason}
                )
    upsert = [
        {
    ...
    return {
        ...
        "data_plane_target": FROZEN_CURRENT_FOLDER_DRIVE_ID,
        "semantic_state_target": BEHAVIOR_CONTROL_FOLDER_DRIVE_ID,
        "purge_before_upsert": purge,
        "upsert_processed_sources": upsert,
        "replace_data_plane_control_files": list(DATA_PLANE_CONTROL_FILE_NAMES),
        "replace_semantic_control_files": list(SEMANTIC_CONTROL_FILE_NAMES),
        "commit_order": [
            "PURGE_STALE_SOURCE_SCOPED_DERIVATIVES",
            "UPSERT_PROCESSED_SOURCE_ARTIFACTS",
            "RECONCILE_DATA_PLANE_RUNTIME",
            "ATOMIC_DATA_PLANE_CONTROL_POINTER_COMMIT",
            "RECONCILE_SEMANTIC_WORK_STATE",
            "ATOMIC_SEMANTIC_LEDGER_AND_WORK_QUEUE_COMMIT",
        ],
        "note": (
            "Gate F2 records the permanent split deployment plan only; canonical "
            "CURRENT and behavior-control state are not mutated by SHADOW mode."
        ),
    }
...
def _unique_result(writer_api, run_folder_id: str) -> dict | None:
    matches = [
        ...
        for item in _list_children(writer_api, run_folder_id)
        if item.get("mimeType") != FOLDER_MIME
        and item.get("name") == MACHINE_RESULT_NAME
    ]
...
def run_governed_delta_machine(*, mode: str = MACHINE_MODE_SHADOW) -> dict:
    """Run the permanent governed dual-state machine under a governed commit policy.

    The production code path keeps DATA-PLANE delta state independent from
    SEMANTIC-RESEARCH work completion. Gate F/F2 exercises this exact path in
    SHADOW mode; no disposable tester engine or hidden semantic classifier exists.
    """
    mode = str(mode).upper()
    ...
    if mode == MACHINE_MODE_CANONICAL:
        raise RuntimeError(
            "CANONICAL_COMMIT_NOT_AUTHORIZED: dual-state shadow reconciliation + owner authority required"
        )

    preflight = run_source_preflight()
    if not preflight.get("pass"):
        raise RuntimeError(
            "SOURCE_PREFLIGHT_HOLD: governed delta machine will not continue"
        )

    reader_api = build_drive_api(read_write=False)
    writer_api = build_drive_api(read_write=True)
    _assert_folder(
        reader_api, FROZEN_CURRENT_FOLDER_DRIVE_ID, CANONICAL_CURRENT_FOLDER_NAME
    )
    _assert_folder(
        writer_api,
        FROZEN_PARITY_STAGING_FOLDER_DRIVE_ID,
    ...
    classification = classify_delta_state(
        preflight=preflight, persistent_state=baseline_persistent
    )
    semantic_runtime = discover_semantic_runtime(reader_api)

    checkpoint, resumed = _find_or_create_run(
        writer_api,
        preflight=preflight,
        persistent=baseline_persistent,
        classification=classification,
        semantic_runtime=semantic_runtime,
    )
    ...
    plan = {
        ...
        "semantic_runtime_status": semantic_runtime.get("status"),
        "semantic_runtime_holds": semantic_runtime.get("holds") or [],
        "canonical_source_count_observed": classification.get("active_source_count"),
        "source_count_is_not_an_invariant": True,
        "data_plane_state_is_not_semantic_completion": True,
        "end_to_end_production_ready": False,
        "classification": classification,
        "canonical_commit_operations": _commit_operations(
            classification, baseline_global
        ),
        "canonical_write_policy": {
            "raw": "READ_ONLY_NEVER_WRITTEN",
            "current": "READ_ONLY_WHILE_GATE_F2_OPEN",
            "behavior_control": "READ_ONLY_WHILE_GATE_F2_OPEN",
            "shadow": "PARITY_STAGING_ONLY",
        },
    }
    if not resumed:
        ...
        checkpoint.update(
            {
                "status": "HOLD",
                "hold": {
                    "reason": "DELTA_STATE_CLASSIFICATION_HOLD",
                    "holds": classification["holds"],
                },
                "updated_at_utc": _iso(),
            }
        )
        checkpoint["checkpoint_sequence"] = int(
            checkpoint.get("checkpoint_sequence", 0)
        ) + 1
        _persist_checkpoint(writer_api, checkpoint)
        result = {
            "pass": False,
    ...
    source_folders = [
        ...
        for item in _list_children(writer_api, run_folder_id)
        if item.get("mimeType") == FOLDER_MIME
        and item.get("name") == "PROCESSED_SOURCES"
    ]
    if len(source_folders) > 1:
        raise RuntimeError("DELTA_MACHINE_PROCESSED_SOURCES_FOLDER_DUPLICATE")
    sources_folder_id = (
        str(source_folders[0]["id"])
        if source_folders
        else _create_folder(
            writer_api, parent_id=run_folder_id, name="PROCESSED_SOURCES"
        )
    )

    row_by_id = source_row_by_id(preflight)
    process_rows = sorted(
        list(classification.get("new_processed", []))
        + list(classification.get("changed_rebuilt", []))
        + list(classification.get("replacement_same_content", [])),
        key=lambda row: (
    ...
    try:
        for index, row in enumerate(process_rows, start=1):
            ...
            if processed.get(source_id, {}).get("pass") is True:
                print(
                    f"RESUME VERIFIED DELTA SOURCE {index}/{len(process_rows)} | {row['source_name']}"
                )
                continue
            selected = row_by_id.get(source_id)
            if selected is None:
                raise RuntimeError(
                    f"DELTA_MACHINE_PREFLIGHT_ROW_MISSING:{source_id}"
                )
            checkpoint.update(
                {
                    "current_source": selected["name"],
                    "next_exact_resume_source": selected["name"],
                    "updated_at_utc": _iso(),
                }
            )
            checkpoint["checkpoint_sequence"] = int(
            ...
            _persist_checkpoint(writer_api, checkpoint)
            print(
                f"PROCESS DELTA SOURCE {index}/{len(process_rows)} | "
                f"{selected['name']} | {action_by_id[source_id]}"
            )
            root, engine_result, manifest = process_source_to_local(
                selected=selected, reader_api=reader_api
            )
            try:
            ...
            if not persisted.get("pass"):
                raise RuntimeError(
                    f"DELTA_MACHINE_SOURCE_EVIDENCE_UPLOAD_FAILED:{selected['name']}"
                )
            processed[source_id] = persisted
            checkpoint["processed_sources"] = processed
            checkpoint["current_source"] = None
            checkpoint["next_exact_resume_source"] = (
                process_rows[index]["source_name"]
    ...
    control_upload = persist_control_bundle(
        writer_api,
        run_folder_id=run_folder_id,
        bundle=bundle,
        stamp=_stamp(),
    ...
    if not control_upload.get("pass"):
        raise RuntimeError(
            "DELTA_MACHINE_CONTROL_BUNDLE_UPLOAD_RECONCILIATION_FAILED"
        )

    semantic_ledger = bundle["PERSISTENT_SEMANTIC_RESEARCH_STATE.json"]
    semantic_work_queue = bundle["AI_SEMANTIC_WORK_QUEUE.json"]
    result = {
        "pass": True,
        **plan,
        ...
        "semantic_scheduler_status": semantic_work_queue.get("scheduler_status"),
        "semantic_work_item_count": semantic_work_queue.get("work_item_count"),
        "semantic_backlog_state": (semantic_ledger.get("watermarks") or {}).get(
            "backlog_state"
        ),
        "semantic_complete": semantic_work_queue.get("work_item_count") == 0
        and semantic_work_queue.get("scheduler_status")
        == "NO_PENDING_SEMANTIC_WORK_FOR_DISCOVERED_SCOPE",
        "end_to_end_production_ready": False,
        "resumed_existing_run": resumed,
        "next_stage": "GATE_F2_DUAL_STATE_SEMANTIC_SCHEDULER_SHADOW_RECONCILIATION",
        "note": (
            "Permanent governed delta machine operating under SHADOW commit policy. "
            "Data-plane readiness is independent from semantic-research completion; "
            "the same durable production modules emit both data controls and the "
            "semantic ledger/work queue without performing semantic interpretation."
        ),
    }
